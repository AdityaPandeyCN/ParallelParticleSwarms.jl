...
# Fix: get correct type by calling remake once (remake changes specialization)
_typed_prob = remake(improb, p = improb.p)
probs = similar(gpu_particles, typeof(_typed_prob), (length(gpu_particles),))
...
for i in 1:maxiters
    ...
    broadcast!(prob_func, probs, Ref(_typed_prob), gpu_particles)
